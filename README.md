...
对于 LLM 的请求，支持下列处置动作：
- **pass**：请求通过，正常放行
- **block**：请求被拦截，返回安全提示信息；风险请求不会发送到模型端
- **hint**：请求被标记，返回提示信息但不阻断

对于 LLM 的响应，支持下列处置动作：
- **pass**：响应通过，正常放行
- **block**：响应被拦截，返回安全提示信息
- **hint**：响应被标记，返回提示信息但不阻断

对于 Tool Call 执行前，支持下列处置动作：
- **pass**：Tool 调用请求通过，正常执行
- **block**：拦截 Tool 调用请求，返回安全提示信息

对于 Tool Call 执行后，支持下列处置动作：
- **pass**：Tool 调用结果检测通过，正常返回
- **block**：拦截 Tool 调用结果，返回安全提示信息
